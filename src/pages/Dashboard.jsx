...
export default function Dashboard() {
    ...
    const [inboundGrouped, setInboundGrouped] = useState([]);
    ...
    function getDataInbound() {
        ...
        .then(res => {
            ...
            const processedData = Object.keys(groupedData).map((date) => ({
                date,
                totalInbound: groupedData[date].reduce((acc, entry) => acc + entry.total_inbound, 0)
            }));
            setInboundGrouped(processedData);
            setCheckProses(true);
        })
        .catch(err => {
            if (err.response.status === 401) {
            ...
            <div className="flex flex-wrap justify-center m-10">
                ...
                {checkProses && (
                    <ResponsiveContainer width="100%" height={400}>
                        <BarChart
                            data={lendingGrouped.map((lend, index) => ({
                                date: lend.date,
                                totalStuff: lend.totalStuff,
                                totalInbound: inboundGrouped[index]?.totalInbound || 0
                            }))}
